...
class ImportPurchaseController extends Controller
{
    ...
    public function index()
    {
    	...
    	$imported_purchase_array = [];
    	foreach ($imported_sales as $sale) {
    		$imported_purchase_array[$sale->import_batch]['import_time'] = $sale->import_time;
    		$imported_purchase_array[$sale->import_batch]['created_by'] = $sale->sales_person->user_full_name;
    		$imported_purchase_array[$sale->import_batch]['invoices'][] = $sale->invoice_no;
    	}

    	$import_fields = $this->__importFields();

    	return view('import_purchase.index')->with(compact('imported_purchase_array', 'import_fields'));
    }
    ...
    public function preview(Request $request)
    {
    	...
    	if ($request->hasFile('sales')) {
    		...
    		return view('import_purchase.preview')->with(compact('parsed_array', 'import_fields', 'file_name', 'business_locations', 'match_array'));
        }
    }
    ...
    public function import(Request $request)
    {
    	...
    	try {
	        ...
	        $this->__importPurchase($formatted_sales_data, $business_id, $location_id);
    ...
    private function __importPurchase($formated_data, $business_id, $location_id)
    {
    	...
    	foreach ($formated_data as $data) {
            ...
            foreach ($data as $line_data) {
                ...
                if (!empty($line_data['item_tax'])) {
                    ...
                    if (empty($tax)) {
                        throw new \Exception(__('lang_v1.import_purchase_tax_not_found', ['row' => $row_index, 'tax_name' => $line_data['item_tax']]));
                    }
                    $tax_id = $tax->id;
    		...
    		$transaction = $this->transactionUtil->createPurchaseTransaction($business_id, $purchase_data, $invoice_total, auth()->user()->id, false);
		//	$this->productUtil->createOrUpdatePurchaseLines($transaction, $purchases, $currency_details, $enable_product_editing);
			// print_r($purchase_lines);
			// die;
    		$this->transactionUtil->createOrUpdateSellLines1($transaction, $purchase_lines, $location_id, false, null, [], false);
    ...
    public function revertPurchaseImport($batch)
    {
        ...
        try {
            ...
            foreach ($purchases as $purchase) {
            	$this->transactionUtil->deleteSale($business_id, $purchase->id);
            }
