$(document).ready(function(){
	...
	*/
	...
	$('#calendar').fullCalendar({
        ...
        dayClick:function( date, jsEvent, view ) {
            ...
            if( clickCount == 2 ){
            	load_schedule_modal();
              	$('form#add_schedule #start_datetime').data("DateTimePicker").date(date).ignoreReadonly(true);
              	$('form#add_schedule #end_datetime').data("DateTimePicker").date(date).ignoreReadonly(true);
            }
	...
	$(document).on('click', '.today_schedule', function() {
		var url = $(this).data('href');
	...
	* related code
	*/
	$(document).on('click', '.schedule_log_add', function() {
		var url = $(this).data('href');
	    $.ajax({
	        method: 'GET',
	        url: url,
	...
	*/
    ...
    $(document).on('change', "#life_stage, #source", function(){
	   var lead_view = urlSearchParam('lead_view');
		if (lead_view == 'kanban') {
		    initializeLeadKanbanBoard();
		} else if(lead_view == 'list_view') {
	...
	function updateLeadLifeStageForKanban(data, el) {
		$.ajax({
	        method: 'GET',
	        dataType: 'json',
	        url: '/crm/contact-leads/' + data.lead_id + '/post-life-stage',
	        data:data,
	        success: function(result){
	            if (result.success) {
	...
	if ($('form#campaign_form').length) {
        ...
        $('select#campaign_type').change(function() {
            ...
            if (campaign_type == 'sms') {
                $('div.sms_div').fadeIn();
                $('div.email_div').fadeOut();
            } else if (campaign_type == 'email') {
                $('div.email_div').fadeIn();
                $('div.sms_div').fadeOut();
            }
        });
	}
...
function get_todays_schedule() {
	$.ajax({
        method: 'GET',
        dataType: "json",
        url: '/crm/contact-todays-schedule',
        success: function(result){
            if (result.success) {
